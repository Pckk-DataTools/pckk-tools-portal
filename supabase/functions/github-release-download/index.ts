...
Deno.serve(async (req) => {
  ...
  if (!payload.tool_asset_id) {
    return new Response("tool_asset_id is required", { status: 400, headers: CORS_HEADERS })
  }
  ...
  try {
    ...
    if (repoError || !repoRow) {
      return new Response("Repository mapping not found", { status: 404, headers: CORS_HEADERS })
    }
    ...
    if (!ghResponse.ok) {
      ...
      return new Response("GitHub download failed", { status: 502, headers: CORS_HEADERS })
    }
    ...
    return new Response(String(error), { status: 500, headers: CORS_HEADERS })
  }
})
