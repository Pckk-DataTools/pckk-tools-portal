...
- 背景は白またはプラチナグレー（薄いグレー）
- メインカラーはパシフィックブルー（濃紺）、アクセントはアースブルー（青）
- 境界線は薄いプラチナグレー
- ヒーロー背景およびページ背景に、プレシジョングリッド（方眼線）と、パシフィックコンサルタンツのロゴモチーフである「重なり合うスクエア（正方形）」の精密な幾何学的ラインアート（SVG）を置く（等高線パターンは廃止）
- 派手さより可読性、安定感、および社会インフラを支える企業としての知性と信頼感を優先する

デザインシステム準拠（2026-04-24 追記、2026-05-20 改定）:

- Stitch プロジェクト `1948335383852346385` の `Pacific Infrastructure Portal` を参照する
- タイポグラフィは見出し `Public Sans`、本文 `Inter` を基本とする
- ツールカードは「白背景 + 1px プラチナボーダー + 低装飾」を基本とする
- 角丸をシャープ（6px〜8px）にし、CAD図面や設計書のような直線的で整ったスタイルとする
- カードのホバー時に「重なり合うスクエア」を想起させるような、スムーズな二重枠線/光彩のアニメーションを適用する
